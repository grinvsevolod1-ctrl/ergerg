  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-8">
        ...
        <p className="text-zinc-400 mt-1">Добавьте Nexik на ваш сайт за пару минут</p>
      </div>
      ...
      <div className="rounded-2xl border border-[#1a1a2e] bg-[#0a0a0f]/80 p-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={copyCode} className="gap-2 border-[#2a2a3e] hover:bg-[#1a1a2e]">
              {copied ? (
                <>
      ...
      <div className="rounded-2xl border border-[#1a1a2e] bg-[#0a0a0f]/80 p-6 mb-8">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={checkInstallation} disabled={!websiteUrl || checkingStatus} className="bg-[#00ffff] text-black hover:bg-[#00ffff]/90 gap-2">
            {checkingStatus ? (
              <>
        ...
        <div className="p-4 border-b border-[#1a1a2e] flex flex-wrap gap-2">
          {Object.entries(platformInstructions).map(([key, platform]) => (
            ...
            >
              ...
              {platform.name}
              {platform.hasOAuth && <div className="w-1.5 h-1.5 rounded-full bg-[#00ff88]" />}
            </button>
          ))}
        </div>
        {selectedPlatform && (
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">Установка на {platformInstructions[selectedPlatform].name}</h3>
              {platformInstructions[selectedPlatform].hasOAuth && (
                <Button onClick={() => startOAuth(selectedPlatform)} disabled={oauthLoading === selectedPlatform} className="bg-[#00ff88] text-black hover:bg-[#00ff88]/90 gap-2">
                  {oauthLoading === selectedPlatform ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                  Автоматически
                  {platformInstructions[selectedPlatform].oauthBeta && <span className="text-[10px] bg-black/20 px-1.5 py-0.5 rounded">BETA</span>}
                </Button>
              )}
            </div>
            <div className="space-y-3 mb-6">
              {platformInstructions[selectedPlatform].steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="w-6 h-6 rounded-full bg-[#00ffff]/20 text-[#00ffff] flex items-center justify-center flex-shrink-0 text-sm font-medium">{i + 1}</span>
                  <p className="text-zinc-300 pt-0.5">{step}</p>
                </div>
              ))}
            </div>
            {platformInstructions[selectedPlatform].docsUrl && (
              <a href={platformInstructions[selectedPlatform].docsUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#00ffff] text-sm hover:underline">
      ...
      <div className="rounded-2xl border border-[#1a1a2e] bg-[#0a0a0f]/80 overflow-hidden mb-8">
        <button onClick={() => setShowCnameInstructions(!showCnameInstructions)} className="w-full p-6 flex items-center justify-between hover:bg-[#12121a]/50 transition-colors">
        ...
        {showCnameInstructions && (
          <div className="p-6 pt-0 border-t border-[#1a1a2e] mt-0">
            <div className="bg-[#12121a] rounded-xl p-4 mb-6">
              <p className="text-sm text-zinc-300 mb-3">Добавьте CNAME запись в DNS вашего домена:</p>
            ...
            {selectedRegistrar && (
              <div className="bg-[#12121a] rounded-xl p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <a href={registrarInstructions[selectedRegistrar].dnsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#00ffff] text-sm hover:underline">
                    Открыть DNS панель
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                </div>
                <div className="space-y-2">
                  {registrarInstructions[selectedRegistrar].steps.map((step, i) => (
                    <div key={i} className="flex items-start gap-3 text-sm">
                      <span className="w-5 h-5 rounded-full bg-[#00ffff]/20 text-[#00ffff] flex items-center justify-center flex-shrink-0 text-xs">{i + 1}</span>
                ...
                <div className="mt-4 p-3 bg-[#0a0a0f] rounded-lg flex items-start gap-2">
                  ...
                  <p className="text-xs text-zinc-400">Изменения DNS могут занять до 24 часов для полного применения. Обычно это происходит за 5-30 минут.</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="rounded-2xl border border-[#ff00aa]/20 bg-gradient-to-br from-[#ff00aa]/5 to-transparent overflow-hidden">
        <button onClick={() => setShowHelpForm(!showHelpForm)} className="w-full p-6 flex items-start gap-4">
          ...
          <div className="flex-1 text-left">
            ...
            <p className="text-zinc-400 text-sm">Мы можем установить виджет за вас бесплатно. Просто оставьте заявку.</p>
          </div>
              ...
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={helpFormData.name} onChange={(e) => setHelpFormData({ ...helpFormData, name: e.target.value })} className="w-full bg-[#12121a] border border-[#2a2a3e] rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#ff00aa]/50" placeholder="Иван" />
                  </div>
                  <div>
                    <label className="block text-sm text-zinc-400 mb-2">Email</label>
                    <input type="email" value={helpFormData.email} onChange={(e) => setHelpFormData({ ...helpFormData, email: e.target.value })} className="w-full bg-[#12121a] border border-[#2a2a3e] rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#ff00aa]/50" placeholder="ivan@example.com" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-zinc-400 mb-2">Адрес сайта</label>
                  <input type="url" value={helpFormData.website} onChange={(e) => setHelpFormData({ ...helpFormData, website: e.target.value })} className="w-full bg-[#12121a] border border-[#2a2a3e] rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#ff00aa]/50" placeholder="https://my-site.com" />
                </div>
                <div>
                  <label className="block text-sm text-zinc-400 mb-2">Комментарий (необязательно)</label>
                  <textarea value={helpFormData.message} onChange={(e) => setHelpFormData({ ...helpFormData, message: e.target.value })} rows={3} className="w-full bg-[#12121a] border border-[#2a2a3e] rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#ff00aa]/50 resize-none" placeholder="Платформа сайта, особенности, пожелания..." />
                </div>
                <Button onClick={submitHelpForm} disabled={!helpFormData.email || !helpFormData.website} className="w-full bg-[#ff00aa] text-white hover:bg-[#ff00aa]/90 gap-2">
                  Отправить заявку
          ...
          {[
            ...
            <div key={pkg.name} className="p-4 rounded-xl bg-[#12121a] border border-[#2a2a3e]">
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { navigator.clipboard.writeText(pkg.cmd) }} className="text-zinc-500 hover:text-white transition-colors">
